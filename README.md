...
Ceci se généralise à l'ensemble des bits $\tau_{\sigma}$, desquels nous déduisons alors que $\left(E_{\alpha\sigma} \ge E_{\beta\sigma}\right)$.
...
Pour le dire autrement, nous savons que $\left(T_{\alpha\sigma} \ge T_{\beta\sigma}\right)$ lorsque $\left(\tau_{\sigma} = 0\right)$, ce qui sous-entend que $T_{\alpha\sigma}$ peut être défini à $0$ comme à $1$.
